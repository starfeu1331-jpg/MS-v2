...
import { trackExport, trackFilter } from '../../services/tracker';
...
// Groupes fixes Nord / Sud (41 = dépôt web, exclu)
const NORD_CODES = new Set(['17','19','22','23','24','26','27','28','32','34','36']);
const SUD_CODES  = new Set(['12','13','14','16','25','29','31','33','35','37','38','39']);
...
// Construire les query params pour la période
function buildPeriodParams(period?: { type: string; value: any }): string {
  if (!period || period.type === 'all') return '';
  return `&periodType=${encodeURIComponent(period.type)}&periodValue=${encodeURIComponent(period.value)}`;
}

interface ZoneChalandiseProps {
  period?: { type: string; value: number | string; label?: string };
  initialStore?: string;
}

export default function ZoneChalandise({ period, initialStore }: ZoneChalandiseProps) {
  const [stores, setStores] = useState<Store[]>([]);
  const [selectedStore, setSelectedStore] = useState<string>(initialStore || ''); // code magasin ou "NORD" / "SUD" / "TOUS"
  const [rawZones, setRawZones] = useState<Zone[]>([]);

  useEffect(() => { if (initialStore) setSelectedStore(initialStore) }, [initialStore])
  ...
  // ── Groupes Nord / Sud (listes fixes) ──
  const storesWithCoords = useMemo(() => stores.filter(s => s.lat && s.lon && s.code !== '41'), [stores]);
  const nordStoreCodes = useMemo(() => storesWithCoords.filter(s => NORD_CODES.has(s.code)).map(s => s.code), [storesWithCoords]);
  const sudStoreCodes = useMemo(() => storesWithCoords.filter(s => SUD_CODES.has(s.code)).map(s => s.code), [storesWithCoords]);
  const allStoreCodes = useMemo(() => storesWithCoords.filter(s => NORD_CODES.has(s.code) || SUD_CODES.has(s.code)).map(s => s.code), [storesWithCoords]);

  // Résoudre les codes magasins à charger selon la sélection
  const resolvedCodes = useMemo((): string[] => {
    if (selectedStore === 'TOUS') return allStoreCodes;
    if (selectedStore === 'NORD') return nordStoreCodes;
    if (selectedStore === 'SUD') return sudStoreCodes;
    if (selectedStore) return [selectedStore];
    return [];
  }, [selectedStore, allStoreCodes, nordStoreCodes, sudStoreCodes]);

  const isGroupSelection = selectedStore === 'TOUS' || selectedStore === 'NORD' || selectedStore === 'SUD';
  ...
  // ── Charger les zones brutes quand le magasin ou la période change ──
  useEffect(() => {
    ...
    const fetchAllStores = async () => {
      try {
        // Agréger les données de tous les magasins sélectionnés par CP
        const aggregated = new Map<string, Zone>();

        // Charger par lots de 5 stores en parallèle pour les groupes
        const STORE_BATCH = 5;
        for (let i = 0; i < resolvedCodes.length; i += STORE_BATCH) {
          const batch = resolvedCodes.slice(i, i + STORE_BATCH);
          const results = await Promise.allSettled(
            batch.map(code =>
              fetch(`/api/stores?action=catchment&storeCode=${encodeURIComponent(code)}${periodParams}`)
                .then(res => res.json())
            )
          );
          ...
          for (const result of results) {
            ...
            const fetched: Zone[] = result.value.data || [];
            for (const zone of fetched) {
              const key = String(zone.cp).trim().padStart(5, '0');
              const existing = aggregated.get(key);
              if (existing) {
                existing.nbClients += zone.nbClients;
                existing.totalCA += zone.totalCA;
                existing.nbTransactions += zone.nbTransactions;
              } else {
                aggregated.set(key, { ...zone });
              }
            }
          }
        }
        ...
        console.log(`✅ ${merged.length} zones brutes (${resolvedCodes.length} magasin(s))`);

        // Enrichir avec population locale
        const enriched = merged.map(zone => {
    ...
    fetchAllStores();
  }, [resolvedCodes.join(','), period?.type, period?.value]);

  // ── Charger stats magasin (uniquement pour un magasin individuel) ──
  useEffect(() => {
    if (!selectedStore || isGroupSelection) { setStoreStats(null); return; }
  ...
  }, [selectedStore, isGroupSelection]);
  ...
  // ── Recalculer les zones quand critères/mode/données changent ──
  // En mode normal: affiche toutes les zones (seuil backend >= 10 tx)
  // En mode per capita: exclut les zones sans population, re-trie entièrement
  //   → les zones affichées sont DIFFÉRENTES (certaines disparaissent, d'autres montent)
  useEffect(() => {
    if (rawZones.length === 0) return;

    setLoading(true);

    // En mode per capita, exclure les zones sans population
    ...
    console.log(`🎯 ${sorted.length} zones (${sortCriterion}${perCapitaMode ? '/hab' : ''})`);
    if (sorted.length > 0) {
      const best = sorted[0];
  ...
  }, [rawZones, sortCriterion, perCapitaMode, getValue]);
  ...
  const exportToExcel = () => {
    ...
    trackExport('xlsx', 'zones', { store: selectedStore });
        ...
        {panelOpen && (
          <div style={{ padding: '16px' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <select
                value={selectedStore}
                onChange={(e) => { setSelectedStore(e.target.value); trackFilter('zones', { store: e.target.value }); }}
                style={{
              ...
              >
                ...
                <option value="TOUS" style={{ fontWeight: 'bold' }}>🌍 Tous les magasins ({allStoreCodes.length})</option>
                <option value="NORD" style={{ fontWeight: 'bold' }}>⬆️ Nord ({nordStoreCodes.length} mag.)</option>
                <option value="SUD" style={{ fontWeight: 'bold' }}>⬇️ Sud ({sudStoreCodes.length} mag.)</option>
            ...
            <div style={{ marginBottom: '14px' }}>
              <button
                ...
                disabled={loading || resolvedCodes.length === 0}
                style={{
                  ...
                  cursor: (loading || resolvedCodes.length === 0) ? 'not-allowed' : 'pointer',
                  opacity: (loading || resolvedCodes.length === 0) ? 0.5 : 1,
                }}
              >
                {getPerCapitaLabel()}
              </button>
            ...
            {zones.length > 0 && (
              <div style={{
                ...
                <div>{zones.length} zones{perCapitaMode ? ' (avec population)' : ''}{isGroupSelection ? ` — ${resolvedCodes.length} magasins` : ''}</div>
                ...
                {loading && <div style={{ color: '#60a5fa', marginTop: '4px' }}><span className="inline-block h-3 w-20 bg-blue-500/20 rounded skel-breath" /></div>}
              </div>
            )}
          </div>
        )}
