    ...
    const [kpis, topProduits, topMagasins, topClients, evolutionMensuelle] = await Promise.all([
      prisma.$queryRaw`
        SELECT 
          ...
          COUNT(DISTINCT carte),
          SUM(ca),
          SUM(CASE WHEN depot != 'WEB' THEN ca ELSE 0 END),
          SUM(CASE WHEN depot = 'WEB' THEN ca ELSE 0 END),
          COUNT(CASE WHEN depot != 'WEB' THEN 1 END),
          COUNT(CASE WHEN depot = 'WEB' THEN 1 END),
          COUNT(DISTINCT CASE WHEN depot != 'WEB' THEN facture END),
          COUNT(DISTINCT CASE WHEN depot = 'WEB' THEN facture END)
        FROM transactions
      `,
      prisma.$queryRaw`
