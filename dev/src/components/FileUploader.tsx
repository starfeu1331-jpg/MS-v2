...
export default function FileUploader({ onDataLoaded }: FileUploaderProps) {
  ...
  const processData = async (
    ...
    for (let i = 0; i < transactions.length; i++) {
      ...
      const ttcKey = rowKeys.find(k => k.includes('T.T.C') || k.includes('TTC') || k.includes('Mt T')) || rowKeys[8]
      ...
      const ttcStr = String(row[ttcKey] || '0').replace(',', '.')
      ...
      const ttc = parseFloat(ttcStr) || 0
      // CA = montant TTC (et non quantite * prix)
      const ca = ttc || (quantite * prix)
    ...
    console.log('  CA total:', (Object.values(processed.familles).reduce((sum: number, f: any) => sum + (Number(f.ca) || 0), 0) as number).toFixed(2), '€')
