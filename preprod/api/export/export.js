...
export default async function handler(req, res) {
  ...
  try {
    ...
    const famillesData = await prisma.$queryRaw`
      ...
      WHERE p.famille IS NOT NULL AND p.famille != '' AND t.depot != '41'
      GROUP BY p.famille
      ORDER BY ca DESC
    `;
    ...
    const clientsData = await prisma.$queryRaw`
      ...
      WHERE t.carte != '0' AND t.depot != '41'
    ...
    const magasinsData = await prisma.$queryRaw`
      ...
      FROM transactions t
      WHERE t.depot IS NOT NULL AND t.depot != '' AND t.depot != '41'
      GROUP BY t.depot
      ORDER BY ca DESC
    `;
